...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['ADD'])) {
        ...
        try {
            ...
            $AC_REGISTER = $_POST['AC_REGISTER'];
            $DESCRIPTION = $_POST['DESCRIPTION'];
            $AC_TYPE = $_POST['AC_TYPE'];
            $STATUS = $_POST['STATUS'];
            $DATE_OF_MANUFACTURE = $_POST['DATE_OF_MANUFACTURE'];
            $HANGAR = $_POST['HANGAR'];
            $COUNTRY_OF_MANUFACTURE = $_POST['COUNTRY_OF_MANUFACTURE'];
            $TAIL_NUMBER = $_POST['TAIL_NUMBER'];
            $AIRCRAFT_SERIAL_NUMBER = $_POST['AIRCRAFT_SERIAL_NUMBER'];
            $FLEET_CODE = $_POST['FLEET_CODE'];

            // Prepare SQL statement to insert data into database
            $stmt = $dbh->prepare("INSERT INTO AIRCRAFT (AIRCRAFT_ID, AC_REGISTER, DESCRIPTION, AC_TYPE, STATUS, DATE_OF_MANUFACTURE, HANGAR, COUNTRY_OF_MANUFACTURE, TAIL_NUMBER, AIRCRAFT_SERIAL_NUMBER, FLEET_CODE) 
                                   VALUES (:AIRCRAFT_ID, :AC_REGISTER, :DESCRIPTION, :AC_TYPE, :STATUS, :DATE_OF_MANUFACTURE, :HANGAR, :COUNTRY_OF_MANUFACTURE, :TAIL_NUMBER, :AIRCRAFT_SERIAL_NUMBER, :FLEET_CODE)");

            // Bind parameters and execute the statement
            $stmt->bindParam(':AIRCRAFT_ID', $AIRCRAFT_ID);
            $stmt->bindParam(':AC_REGISTER', $AC_REGISTER);
            $stmt->bindParam(':DESCRIPTION', $DESCRIPTION);
            $stmt->bindParam(':AC_TYPE', $AC_TYPE);
            $stmt->bindParam(':STATUS', $STATUS);
            $stmt->bindParam(':DATE_OF_MANUFACTURE', $DATE_OF_MANUFACTURE);
            $stmt->bindParam(':HANGAR', $HANGAR);
            $stmt->bindParam(':COUNTRY_OF_MANUFACTURE', $COUNTRY_OF_MANUFACTURE);
            $stmt->bindParam(':TAIL_NUMBER', $TAIL_NUMBER);
            $stmt->bindParam(':AIRCRAFT_SERIAL_NUMBER', $AIRCRAFT_SERIAL_NUMBER);
            $stmt->bindParam(':FLEET_CODE', $FLEET_CODE);
...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Search For Aircraft</h5>
            <p class="card-description">Enter the ID you want to search for:</p>
            <form action="\Project\MAIN temp\template\pages\basic\aircraft.php" method="post">
                <div class="input-group mb-3">
                    <input type="text" class="form-control" name="search_id" placeholder="Enter aircraft ID to search">
...
$password = 'YOUR_PASSWORD'; // Oracle password
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_id'])) {
                ...
                try {
                    ...
                    $stmt = $dbh->prepare("SELECT AIRCRAFT_ID, AC_REGISTER, DESCRIPTION, AC_TYPE, STATUS, DATE_OF_MANUFACTURE, HANGAR, COUNTRY_OF_MANUFACTURE, TAIL_NUMBER, AIRCRAFT_SERIAL_NUMBER, FLEET_CODE FROM AIRCRAFT WHERE AIRCRAFT_ID = :search_id");
                    $stmt->bindParam(':search_id', $search_id);
                    $stmt->execute();

                    // Display the search results
                    echo "<div style='overflow: auto; width: 1250px; height: 170px;'>";
                    echo "<table border='1' class='AIRCRAFT'>";
                    echo "<tr><th>AIRCRAFT_ID</th><th>AC_REGISTER</th><th>DESCRIPTION</th><th>AC_TYPE</th><th>STATUS</th><th>DATE_OF_MANUFACTURE</th><th>HANGAR</th><th>COUNTRY_OF_MANUFACTURE</th><th>TAIL_NUMBER</th><th>AIRCRAFT_SERIAL_NUMBER</th><th>FLEET_CODE</th></tr>";
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        echo "<tr><td>{$row['AIRCRAFT_ID']}</td><td>{$row['AC_REGISTER']}</td><td>{$row['DESCRIPTION']}</td><td>{$row['AC_TYPE']}</td><td>{$row['STATUS']}</td><td>{$row['DATE_OF_MANUFACTURE']}</td><td>{$row['HANGAR']}</td><td>{$row['COUNTRY_OF_MANUFACTURE']}</td><td>{$row['TAIL_NUMBER']}</td><td>{$row['AIRCRAFT_SERIAL_NUMBER']}</td><td>{$row['FLEET_CODE']}</td></tr>";
                    }
                    echo "</table>";
                    echo "</div>";
...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Delete Aircraft with ID</h5>
...
$password = 'YOUR_PASSWORD'; // Oracle password
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_id'])) {
                ...
                try {
                    ...
                    $stmt = $dbh->prepare("DELETE FROM AIRCRAFT WHERE AIRCRAFT_ID = :delete_id");
...
$password = 'YOUR_PASSWORD'; // Oracle password
                ...
                try {
                    ...
                    // Query to fetch aircraft attributes from the database
                    $stmt = $dbh->query("SELECT AIRCRAFT_ID, AC_REGISTER, DESCRIPTION, AC_TYPE, STATUS, DATE_OF_MANUFACTURE, HANGAR, COUNTRY_OF_MANUFACTURE, TAIL_NUMBER, AIRCRAFT_SERIAL_NUMBER, FLEET_CODE FROM AIRCRAFT");
...
<div class="container mt-5">
    <div class="card">
        <div class="card-body">
            ...
            <form action="/Project/MAIN temp/template/pages/basic/aircraft.php" method="POST" class="forms-sample" onsubmit="return handleFormSubmit()">
                ...
                <div class="form-group">
                    <label for="AC_REGISTER" class="form-label"><h5 class="attribute-label">AC REGISTER</h5></label>
                    <input type="text" name="AC_REGISTER" class="form-control" id="AC_REGISTER" placeholder="Enter Aircraft Register">
                </div>
                <div class="form-group">
                    <label for="DESCRIPTION" class="form-label"><h5 class="attribute-label">DESCRIPTION</h5></label>
                    <input type="text" name="DESCRIPTION" class="form-control" id="DESCRIPTION" placeholder="Enter Description">
                </div>
                <div class="form-group">
                    <label for="AC_TYPE" class="form-label"><h5 class="attribute-label">AC TYPE</h5></label>
                    <input type="text" name="AC_TYPE" class="form-control" id="AC_TYPE" placeholder="Enter Aircraft Type">
                </div>
                <div class="form-group">
    ...
    <select name="STATUS" class="form-control" id="STATUS">
        ...
        <option value="NMC">NMC</option>
                ...
                <div class="form-group">
                    <label for="DATE_OF_MANUFACTURE" class="form-label"><h5 class="attribute-label">DATE OF MANUFACTURE</h5></label>
                    <input type="date" name="DATE_OF_MANUFACTURE" class="form-control" id="DATE_OF_MANUFACTURE">
                </div>
                <div class="form-group">
                    <label for="HANGAR" class="form-label"><h5 class="attribute-label">HANGAR</h5></label>
                    <input type="text" name="HANGAR" class="form-control" id="HANGAR" placeholder="Enter Hangar Location">
                </div>
                <div class="form-group">
    <label for="COUNTRY_OF_MANUFACTURE" class="form-label"><h5 class="attribute-label">COUNTRY OF MANUFACTURE</h5></label>
    <select name="COUNTRY_OF_MANUFACTURE" class="form-control" id="COUNTRY_OF_MANUFACTURE">
        ...
        <option value="France">France</option>
                ...
                <div class="form-group">
                    <label for="TAIL_NUMBER" class="form-label"><h5 class="attribute-label">TAIL NUMBER</h5></label>
                    <input type="text" name="TAIL_NUMBER" class="form-control" id="TAIL_NUMBER" placeholder="Enter Tail Number">
                </div>
                <div class="form-group">
                    <label for="AIRCRAFT_SERIAL_NUMBER" class="form-label"><h5 class="attribute-label">AIRCRAFT SERIAL NUMBER</h5></label>
                    <input type="text" name="AIRCRAFT_SERIAL_NUMBER" class="form-control" id="AIRCRAFT_SERIAL_NUMBER" placeholder="Enter Aircraft Serial Number">
                </div>
                <div class="form-group">
                    <label for="FLEET_CODE" class="form-label"><h5 class="attribute-label">FLEET CODE</h5></label>
                    <input type="text" name="FLEET_CODE" class="form-control" id="FLEET_CODE" placeholder="Enter Fleet Code">
                </div>
...
<script>
    function handleFormSubmit() {
        ...
        var acRegister = document.getElementById("AC_REGISTER").value;
        var description = document.getElementById("DESCRIPTION").value;
        var acType = document.getElementById("AC_TYPE").value;
        var status = document.getElementById("STATUS").value;
        var dateOfManufacture = document.getElementById("DATE_OF_MANUFACTURE").value;
        var hangar = document.getElementById("HANGAR").value;
        var countryOfManufacture = document.getElementById("COUNTRY_OF_MANUFACTURE").value;
        var tailNumber = document.getElementById("TAIL_NUMBER").value;
        var aircraftSerialNumber = document.getElementById("AIRCRAFT_SERIAL_NUMBER").value;
        var fleetCode = document.getElementById("FLEET_CODE").value;

        // Check if any of the fields are empty
        if (aircraftID.trim() === '' || acRegister.trim() === '' || description.trim() === '' || acType.trim() === '' || status.trim() === '' || dateOfManufacture.trim() === '' || hangar.trim() === '' || countryOfManufacture.trim() === '' || tailNumber.trim() === '' || aircraftSerialNumber.trim() === '' || fleetCode.trim() === '') {
            alert("Please fill in all fields.");
            return false; // Prevent form submission
        }
       
        // Handle the form submission here, if needed
        alert("Your Aircraft is added.");

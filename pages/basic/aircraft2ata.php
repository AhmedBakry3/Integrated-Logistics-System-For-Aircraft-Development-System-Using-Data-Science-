...
<style>
    ...
    .AIRCRAFT2ATA tr:last-child td {
        border-bottom: none; /* Remove bottom border from last row */
    }

    .AIRCRAFT2ATA tr:nth-child(even) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['ADD'])) {
        ...
        try {
            ...
            $stmt = $dbh->prepare("INSERT INTO ATA (ATA_CHAPTER, AIRCRAFT_ID, DESCRIPTION) VALUES (:ATA_CHAPTER, :AIRCRAFT_ID, :DESCRIPTION)");
...
<div class="card">
    <div class="card-body">
        <h5 class="card-title">Search For ATA Chapter</h5>
        <p class="card-description">Enter the ID you want to search for:</p>
        <form action="\Project\MAIN temp\template\pages\basic\aircraft2ata.php" method="post">
            <div class="input-group mb-3">
                <input type="text" class="form-control" name="search_id" placeholder="Enter chapter to search">
...
$password = 'YOUR_PASSWORD'; // Oracle password
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_id'])) {
            ...
            try {
              ...
              $stmt = $dbh->prepare("SELECT ATA_CHAPTER, AIRCRAFT_ID, DESCRIPTION FROM ATA WHERE ATA_CHAPTER = :search_id");
              $stmt->bindParam(':search_id', $search_id);
              $stmt->execute();

              // Display the search results
              echo "<div style='overflow: auto; width: 1250px; height: 110px;'>";
              echo "<table border='1' class='AIRCRAFT2ATA'>";
              echo "<tr><th>ATA_CHAPTER</th><th>AIRCRAFT_ID</th><th>DESCRIPTION</th></tr>";
              while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                  echo "<tr><td>{$row['ATA_CHAPTER']}</td><td>{$row['AIRCRAFT_ID']}</td><td>{$row['DESCRIPTION']}</td></tr>";
              }
              echo "</table>";
              echo "</div>";
...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Delete ATA with ID</h5>
...
$password = 'YOUR_PASSWORD'; // Oracle password
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_id'])) {
                ...
                try {
                    ...
                    $stmt = $dbh->prepare("DELETE FROM ATA WHERE ATA_CHAPTER = :delete_id");
...
<div class="card">
    <div class="card-body">
        <div class="input-group mb-3">
            <form action="\Project\MAIN temp\template\pages\basic\ata.php" method="post">
                <?php
$hostname = 'YOUR_HOSTNAME'; // Hostname of the Oracle server
$port = 'YOUR_PORT'; // Oracle listener port
$service_name = 'YOUR_SERVICE_NAME'; // Service name or SID of the Oracle database instance
...
$password = 'YOUR_PASSWORD'; // Oracle password
                ...
                try {
                    ...
                    // Query to fetch ATA attributes from the database
                    $stmt = $dbh->query("SELECT ATA_CHAPTER, AIRCRAFT_ID, DESCRIPTION FROM ATA");
...
<div class="container mt-5">
    <div class="card">
        <div class="card-body">
            ...
            <form action="/Project/MAIN temp/template/pages/basic/aircraft2ata.php" method="POST" class="forms-sample" onsubmit="return handleFormSubmit()">
                <div class="form-group">
                    <label for="ATA_ID" class="form-label"><h5 class="attribute-label">ATA CHAPTER</h5></label>
                    <input type="text" name="ATA_CHAPTER" class="form-control" id="ATA_CHAPTER" placeholder="Enter ATA ID">
                </div>
                <div class="form-group">
...
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
  ...
  <script>
    function handleFormSubmit() {
        ...
        if (ataChapter.trim() === '' || aircraftID.trim() === '' || description.trim() === '') {

...
<!-- start of script button -->
  <!-- <script>
    function displayPopupMessageAndRedirect() {
        ...
        window.location.href = 'http://localhost/project/MAIN%20temp/template/pages/basic/sp.php';
    }
</script> -->
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['ADD'])) {
        ...
        try {
            ...
            $stmt = $dbh->prepare("INSERT INTO SP (SP_ID, AIRCRAFT_ID, SUBJECT,ISSUE_DATE,REVISION_DATE) VALUES (:SP_ID, :AIRCRAFT_ID, :SUBJECT, :ISSUE_DATE, :REVISION_DATE)");
...
<div class="card">
    <div class="card-body">
        <h5 class="card-title">Search for SP ID</h5>
        <p class="card-description">Enter the ID you want to search for:</p>
        <form action="\Project\MAIN temp\template\pages\basic\sp.php" method="post">
...
$password = 'YOUR_PASSWORD'; // Oracle password
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_id'])) {
            ...
            try {
                ...
                $stmt = $dbh->prepare("SELECT SP_ID, AIRCRAFT_ID, SUBJECT,ISSUE_DATE,REVISION_DATE FROM SP WHERE SP_ID = :search_id");
                $stmt->bindParam(':search_id', $search_id);
                $stmt->execute();

                // Display the search results
                echo "<div style='overflow: auto; width: 1250px; height: 150PX;'>";
                echo "<table border='1' class='SP'>";
                echo "<tr><th>SP_ID</th><th>AIRCRAFT_ID</th><th>SUBJECT</th><th>ISSUE_DATE</th><th>REVISION_DATE</th></tr>";
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    echo "<tr><td>{$row['SP_ID']}</td><td>{$row['AIRCRAFT_ID']}</td><td>{$row['SUBJECT']}</td><td>{$row['ISSUE_DATE']}</td><td>{$row['REVISION_DATE']}</td></tr>";
                }
                echo "</table>";
                echo "</div>";
...
$password = 'YOUR_PASSWORD'; // Oracle password
                ...
                try {
                    ...
                    // Query to fetch SP attributes from the database
                    $stmt = $dbh->query("SELECT SP_ID, AIRCRAFT_ID, SUBJECT,ISSUE_DATE,REVISION_DATE FROM SP"); // Replace 'SP' with your actual table name
...
<div class="container mt-5">
    <div class="card">
        <div class="card-body">
            ...
            <form action="/Project/MAIN temp/template/pages/basic/sp.php" method="POST" class="forms-sample" onsubmit="return handleFormSubmit()">
                <div class="form-group">
                    <label for="SP_ID" class="form-label"><h5 class="attribute-label">SP ID</h5></label>
                    <input type="text" name="SP_ID" class="form-control" id="SP_ID" placeholder="Enter SP ID">
                </div>
                <div class="form-group">
...
<script>
    function handleFormSubmit() {
        ...
        if (spID.trim() === '' || aircraftID.trim() === '' || subject.trim() === '' || issueDate.trim() === '' || revisionDate.trim() === '') {
            alert("Please fill in all fields.");
            return false; // Prevent form submission
        }
       
        // Handle the form submission here, if needed
        alert("Your SP is added.");

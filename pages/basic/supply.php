...
<!-- start of script button -->
  <!-- <script>
    function displayPopupMessageAndRedirect() {
        ...
        alert('your Supply is Added');

        // Redirect the user to a local URL when they click "OK"
        window.location.href = 'http://localhost/project/MAIN%20temp/template/pages/basic/supply.php';
    }
</script> -->
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['ADD'])) {
        ...
        try {
            ...
            $SUPPLY_COMPANY_NAME = $_POST['SUPPLY_COMPANY_NAME'];

            // Prepare SQL statement to insert data into database
            $stmt = $dbh->prepare("INSERT INTO SUPPLY (SUPPLY_ID, COUNTRY_OF_MANUFACTURE, SUPPLY_COMPANY_NAME) VALUES (:SUPPLY_ID,:COUNTRY_OF_MANUFACTURE, :SUPPLY_COMPANY_NAME)");

            // Bind parameters and execute the statement
            $stmt->bindParam(':SUPPLY_ID', $SUPPLY_ID);
         
            $stmt->bindParam(':COUNTRY_OF_MANUFACTURE', $COUNTRY_OF_MANUFACTURE);
            $stmt->bindParam(':SUPPLY_COMPANY_NAME', $SUPPLY_COMPANY_NAME);
            $stmt->execute();
...
<div class="card">
    <div class="card-body">
        <h5 class="card-title">Search For Supply ID</h5>
        <p class="card-description">Enter the ID you want to search for:</p>
        <form action="\Project\MAIN temp\template\pages\basic\supply.php" method="post">
...
$password = 'YOUR_PASSWORD'; // Oracle password
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_id'])) {
                    ...
                    try {
                        ...
                        $stmt = $dbh->prepare("SELECT SUPPLY_ID, COUNTRY_OF_MANUFACTURE, SUPPLY_COMPANY_NAME FROM SUPPLY WHERE SUPPLY_ID = :search_id");
                        $stmt->bindParam(':search_id', $search_id);
                        $stmt->execute();

                        // Display the search results
                        echo "<div style='overflow: auto; width: 1250px; height: 130px;'>";
                        echo "<table border='1' class='SUPPLY'>";
                        echo "<tr><th>SUPPLY_ID</th><th>COUNTRY_OF_MANUFACTURE</th><th>SUPPLY_COMPANY_NAME</th></tr>";
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            echo "<tr><td>{$row['SUPPLY_ID']}</td><td>{$row['COUNTRY_OF_MANUFACTURE']}</td><td>{$row['SUPPLY_COMPANY_NAME']}</td></tr>";
                        }
                        echo "</table>";
                        echo "</div>";
...
$password = 'YOUR_PASSWORD'; // Oracle password
                ...
                try {
                    ...
                    // Query to fetch supply attributes from the database
                    $stmt = $dbh->query("SELECT SUPPLY_ID, COUNTRY_OF_MANUFACTURE, SUPPLY_COMPANY_NAME FROM SUPPLY");
...
<div class="container mt-5">
    <div class="card">
        <div class="card-body">
            ...
            <form action="/Project/MAIN temp/template/pages/basic/supply.php" method="POST" class="forms-sample" onsubmit="return handleFormSubmit()">
                <div class="form-group">
                    <label for="SUPPLY_ID" class="form-label"><h5 class="attribute-label">SUPPLY ID</h5></label>
                    <input type="text" name="SUPPLY_ID" class="form-control" id="SUPPLY_ID" placeholder="Enter Supply ID">
                </div>
                <div class="form-group">
                    <label for="COUNTRY_OF_MANUFACTURE" class="form-label"><h5 class="attribute-label">COUNTRY_OF_MANUFACTURE</h5></label>
                    <input type="text" name="COUNTRY_OF_MANUFACTURE" class="form-control" id="COUNTRY_OF_MANUFACTURE" placeholder="Enter Country of Manufactury">
                </div>
                <div class="form-group">
                    <label for="SUPPLY_COMPANY_NAME" class="form-label"><h5 class="attribute-label">SUPPLY COMPANY NAME</h5></label>
                    <input type="text" name="SUPPLY_COMPANY_NAME" class="form-control" id="SUPPLY_COMPANY_NAME" placeholder="Enter Supply Company Name">
                </div>
...
<script>
    function handleFormSubmit() {
        ...
        var supplyCompanyName = document.getElementById("SUPPLY_COMPANY_NAME").value;

        // Check if any of the fields are empty
        if (supplyID.trim() === '' || countryofmanufacture.trim() === '' || supplyCompanyName.trim() === '') {

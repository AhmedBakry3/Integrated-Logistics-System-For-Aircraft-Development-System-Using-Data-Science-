...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Search For Flight ID</h5>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_id'])) {
                ...
                try {
                  ...
                  $password = 'YOUR_PASSWORD'; // Oracle password
                    ...
                    $stmt = $dbh->prepare("SELECT FLIGHT_ID, AIRCRAFT_ID, STATUS, TIME_OF_DEPARTURE, TIME_OF_ARRIVAL, DURATION, START_DATE, END_DATE, FROM_, TO_ FROM FLIGHTS WHERE FLIGHT_ID = :search_id");
                    $stmt->bindParam(':search_id', $search_id);
                    $stmt->execute();

                    // Display the search results
                    echo "<div style='overflow: auto; width: 1250px; height: 200px;'>";
                    echo "<table border='1' class='flights'>";
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                      ...
                      // Check if status is not 'finished' to display remaining time
...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Delete Flights with ID</h5>
       ...
       $password = 'YOUR_PASSWORD'; // Oracle password
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_id'])) {
                ...
                try {
                    ...
                    $stmt = $dbh->prepare("DELETE FROM FLIGHTS WHERE FLIGHT_ID = :delete_id");
            ...
            <div class="card-body">
                <form action="\Project\MAIN temp\template\pages\dynamic\flights.php" method="post">
                    ...
                    try {
                        ...
                        // Set PDO error mode to exception
                        $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                        // Query to fetch FLIGHTS attributes where status is 'finished'
                        $stmt = $dbh->prepare("SELECT FLIGHT_ID, AIRCRAFT_ID, STATUS, TIME_OF_DEPARTURE, TIME_OF_ARRIVAL, START_DATE, END_DATE, FROM_, TO_ FROM FLIGHTS WHERE STATUS = 'finished'");
                        $stmt->execute();
                        // Store rows in an array
                        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
                        // Display FLIGHTS attributes
                        echo "<div style='overflow: auto; width: 1250px; height: 300px;'>";
                        echo "<table border='1' class='flights' id='finished_flight_table'>";
                        echo "<tr><th>FLIGHT_ID</th><th>AIRCRAFT_ID</th><th>STATUS</th><th>TIME_OF_DEPARTURE</th><th>TIME_OF_ARRIVAL</th><th>DURATION</th><th>START_DATE</th><th>END_DATE</th><th>FROM_</th><th>TO_</th></tr>";
                        foreach ($rows as $row) {
                            $departureTimeUnix = strtotime($row['TIME_OF_DEPARTURE']);
                            $arrivalTimeUnix = strtotime($row['TIME_OF_ARRIVAL']);
                            $duration = $arrivalTimeUnix - $departureTimeUnix;
...
<div class="container mt-5">
    <div class="card">
        <div class="card-body">
            ...
            <form action="/Project/MAIN temp/template/pages/dynamic/flights.php" method="POST" class="forms-sample" onsubmit="return handleFormSubmit()">
                <div class="form-group">
                    <label for="FLIGHT_ID" class="form-label"><h5 class="attribute-label">FLIGHT ID</h5></label>
                    <input type="text" name="FLIGHT_ID" class="form-control" id="FLIGHT_ID" placeholder="Enter Flight ID">
                </div>
                <div class="form-group">
    ...
    <select name="STATUS" class="form-control" id="STATUS">
        <option value="unfinished">Unfinished</option>
                ...
                <div class="form-group">
                    <label for="TIME_OF_DEPARTURE" class="form-label"><h5 class="attribute-label">TIME OF DEPARTURE</h5></label>
                    <input type="time" name="TIME_OF_DEPARTURE" class="form-control" id="TIME_OF_DEPARTURE">
                </div>
                <div class="form-group">
                    <label for="TIME_OF_ARRIVAL" class="form-label"><h5 class="attribute-label">TIME OF ARRIVAL</h5></label>
                    <input type="time" name="TIME_OF_ARRIVAL" class="form-control" id="TIME_OF_ARRIVAL">
                </div>
                <div class="form-group">
                    <label for="FROM_" class="form-label"><h5 class="attribute-label">FROM</h5></label>
                    <input type="text" name="FROM_" class="form-control" id="FROM_" placeholder="Enter Departure Location">
                </div>
                <div class="form-group">
                    <label for="TO_" class="form-label"><h5 class="attribute-label">TO</h5></label>
                    <input type="text" name="TO_" class="form-control" id="TO_" placeholder="Enter Arrival Location">
                </div>
...
<script>
    function handleFormSubmit() {
        ...
        var departureTime = document.getElementById("TIME_OF_DEPARTURE").value;
        var arrivalTime = document.getElementById("TIME_OF_ARRIVAL").value;
        var from = document.getElementById("FROM_").value;
        var to = document.getElementById("TO_").value;

        // Check if any of the fields are empty
        if (flightID.trim() === '' || aircraftID.trim() === '' || status.trim() === '' || departureTime.trim() === '' || arrivalTime.trim() === ''  || from.trim() === '' || to.trim() === '') {
            alert("Please fill in all fields.");
            return false; // Prevent form submission
        }
       
        // Handle the form submission here, if needed
        alert("Your flight information is added.");

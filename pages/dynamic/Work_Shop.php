...
<!-- start of script button -->
  <!-- <script>
    function displayPopupMessageAndRedirect() {
        ...
        window.location.href = 'http://localhost/project/MAIN%20temp/template/pages/basic/work_shop.php';
    }
</script> -->
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['ADD'])) {
        ...
        try {
            ...
            $WORK_SHOP_ID = $_POST['WORK_SHOP_ID'];
            $HANGAR = $_POST['HANGAR'];

            // Prepare SQL statement to insert data into database
            $stmt = $dbh->prepare("INSERT INTO WORK_SHOP (TOOLS, WORK_ORDER_ID, WORK_SHOP_ID, HANGAR) VALUES (:TOOLS, :WORK_ORDER_ID, :WORK_SHOP_ID, :HANGAR)");

            // Bind parameters and execute the statement
            $stmt->bindParam(':TOOLS', $TOOLS);
            $stmt->bindParam(':WORK_ORDER_ID', $WORK_ORDER_ID);
            $stmt->bindParam(':WORK_SHOP_ID', $WORK_SHOP_ID);
...
<div class="card">
    <div class="card-body">
        <h5 class="card-title">Search For Workshop ID</h5>
        <p class="card-description">Enter the ID you want to search for:</p>
        <form action="\Project\MAIN temp\template\pages\dynamic\Work_Shop.php" method="post">
     ...
     $password = 'YOUR_PASSWORD'; // Oracle password
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_id'])) {
            ...
            try {
                ...
                $stmt = $dbh->prepare("SELECT TOOLS, WORK_ORDER_ID, WORK_SHOP_ID, HANGAR FROM WORK_SHOP WHERE WORK_SHOP_ID = :search_id");
                $stmt->bindParam(':search_id', $search_id);
                $stmt->execute();

                // Display the search results
                echo "<div style='overflow: auto; width: 1250px; height: 130px;'>";
                echo "<table border='1' class='WORKSHOP'>";
                echo "<tr><th>TOOLS</th><th>WORK_ORDER_ID</th><th>WORK_SHOP_ID</th><th>HANGAR</th></tr>";
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    echo "<tr><td>{$row['TOOLS']}</td><td>{$row['WORK_ORDER_ID']}</td><td>{$row['WORK_SHOP_ID']}</td><td>{$row['HANGAR']}</td></tr>";
                }
                echo "</table>";
                echo "</div>";
...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Delete Tool with tool name</h5>
            <p class="card-description">Enter the Tool name you want to delete:</p>
            <form action="\Project\MAIN temp\template\pages\dynamic\Work_shop.php" method="post">
                <div class="input-group mb-3">
         ...
         $password = 'YOUR_PASSWORD'; // Oracle password
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_id'])) {
                ...
                try {
                    ...
                    $stmt = $dbh->prepare("DELETE FROM WORK_SHOP WHERE TOOLS = :delete_id");
...
<div class="card">
    <div class="card-body">
        <div class="input-group mb-3">
            <form action="\Project\MAIN temp\template\pages\basic\Work_Shop.php" method="post">
                <?php
            $hostname = 'YOUR_HOSTNAME'; // Hostname of the Oracle server
            $port = 'YOUR_PORT'; // Oracle listener port
            $service_name = 'YOUR_SERVICE_NAME'; // Service name or SID of the Oracle database instance
            ...
            $password = 'YOUR_PASSWORD'; // Oracle password
                try {
                    ...
                    // Query to fetch workshop attributes from the database
                    $stmt = $dbh->query("SELECT TOOLS, WORK_ORDER_ID, WORK_SHOP_ID, HANGAR FROM WORK_SHOP");

                    // Display workshop attributes in a table
...
<div class="container mt-5">
    <div class="card">
        <div class="card-body">
            ...
            <form action="/Project/MAIN temp/template/pages/dynamic/Work_Shop.php" method="POST" class="forms-sample" onsubmit="return handleFormSubmit()">
            <form action="/Project/MAIN temp/template/pages/dynamic/Work_Shop.php" method="POST" class="forms-sample" onsubmit="return handleFormSubmit()">
    <div class="form-group">
        <label for="TOOLS">TOOLS</label>
        <input type="text" name="TOOLS" class="form-control" id="TOOLS" placeholder="Enter Tools">
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        <label for="WORK_SHOP_ID">WORKSHOP ID</label>
        <input type="text" name="WORK_SHOP_ID" class="form-control" id="WORK_SHOP_ID" placeholder="Enter Workshop ID">
    </div>
    <div class="form-group">
        <label for="HANGAR">Hangar Name</label>
        <input type="text" name="HANGAR" class="form-control" id="HANGAR" placeholder="Enter Hangar Name">
    </div>
...
<script>
    function handleFormSubmit() {
        ...
        var workshopID = document.getElementById("WORK_SHOP_ID").value;
        var hangar = document.getElementById("HANGAR").value;

        // Check if any of the fields are empty
        if (tools.trim() === '' || workOrderID.trim() === '' || workshopID.trim() === '' || hangar.trim() === '') {
            alert("Please fill in all fields.");
            return false; // Prevent form submission
        }
       
        // Handle the form submission here if needed (e.g., AJAX request)
        alert("Your Workshop Tool is being added.");

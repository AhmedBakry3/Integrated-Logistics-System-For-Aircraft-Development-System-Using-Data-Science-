...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['ADD'])) {
        ...
        try {
            ...
            $MAINTENANCE_PROGRAM = $_POST['MAINTENANCE_PROGRAM']; // Updated variable name
            // Prepare SQL statement to insert data into database
            $stmt = $dbh->prepare("INSERT INTO CHECK_ (CHECK_ID, WORK_ORDER_ID, AIRCRAFT_TYPE, PLANNED_START, PLANNED_END, DESCRIPTION, MAINTENANCE, MAINTENANCE_PROGRAM) 
            VALUES (:CHECK_ID, :WORK_ORDER_ID, :AIRCRAFT_TYPE, :PLANNED_START, :PLANNED_END, :DESCRIPTION, :MAINTENANCE, :MAINTENANCE_PROGRAM)");
            // Bind parameters and execute the statement
            $stmt->bindParam(':CHECK_ID', $CHECK_ID);
            $stmt->bindParam(':WORK_ORDER_ID', $WORK_ORDER_ID);
            $stmt->bindParam(':AIRCRAFT_TYPE', $AIRCRAFT_TYPE);
            $stmt->bindParam(':PLANNED_START', $PLANNED_START);
            $stmt->bindParam(':PLANNED_END', $PLANNED_END);
            $stmt->bindParam(':DESCRIPTION', $DESCRIPTION);
            $stmt->bindParam(':MAINTENANCE', $MAINTENANCE);
            $stmt->bindParam(':MAINTENANCE_PROGRAM', $MAINTENANCE_PROGRAM); // Updated binding
            $stmt->execute();
...
<div class="card">
    <div class="card-body">
        <h5 class="card-title">Search for Check ID</h5>
        <p class="card-description">Enter the ID you want to search for:</p>
        <form action="\Project\MAIN temp\template\pages\dynamic\check.php" method="post">
       ...
       $password = 'YOUR_PASSWORD'; // Oracle password
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_id'])) {
            ...
            try {
                ...
                $stmt = $dbh->prepare("SELECT CHECK_ID, WORK_ORDER_ID, AIRCRAFT_TYPE,PLANNED_START,PLANNED_END,DESCRIPTION,MAINTENANCE,MAINTENANCE_PROGRAM FROM CHECK_ WHERE CHECK_ID = :search_id");
                $stmt->bindParam(':search_id', $search_id);
                $stmt->execute();

                // Display the search results
                echo "<div style='overflow: auto; width: 1250px; height: 170px;'>";
                echo "<table border='1' class='CHECK_TABLE'>";
                echo "<tr><th>CHECK_ID</th><th>WORK_ORDER_ID</th><th>AIRCRAFT_TYPE</th><th>PLANNED_START</th><th>PLANNED_END</th><th>DESCRIPTION</th><th>MAINTENANCE</th><th>MAINTENANCE_PROGRAM</th></tr>";
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    echo "<tr><td>{$row['CHECK_ID']}</td><td>{$row['WORK_ORDER_ID']}</td><td>{$row['AIRCRAFT_TYPE']}</td><td>{$row['PLANNED_START']}</td><td>{$row['PLANNED_END']}</td><td>{$row['DESCRIPTION']}</td><td>{$row['MAINTENANCE']}</td><td>{$row['MAINTENANCE_PROGRAM']}</td></tr>";
                }
                echo "</table>";
                echo "</div>";
...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Delete Check with ID</h5>
           ...
           $password = 'YOUR_PASSWORD'; // Oracle password
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_id'])) {
                ...
                try {
                    ...
                    $stmt = $dbh->prepare("DELETE FROM CHECK_ WHERE CHECK_ID = :delete_id");
            ...
            <form action="\Project\MAIN temp\template\pages\dynamic\check.php" method="post">
             ...
             $password = 'YOUR_PASSWORD'; // Oracle password
                try {
                    ...
                    // Set PDO error mode to exception
                    $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                    // Query to fetch all attributes from the database
                    $stmt = $dbh->query("SELECT CHECK_ID, WORK_ORDER_ID, AIRCRAFT_TYPE, PLANNED_START, PLANNED_END, DESCRIPTION, MAINTENANCE, MAINTENANCE_PROGRAM FROM CHECK_");
                    // Store rows in an array
                    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    // Display CHECK attributes
                    ...
                    foreach ($rows as $row) {
                        ...
                        echo "<td>{$row['PLANNED_END']}</td>";
                        echo "<td>{$row['DESCRIPTION']}</td>";
                        echo "<td>{$row['MAINTENANCE']}</td>";
                        echo "<td>{$row['MAINTENANCE_PROGRAM']}</td>";
                        echo "</tr>";
                    }
                    echo "</table>";
                    echo "</div>";
...
<div class="container mt-5">
    <div class="card">
        <div class="card-body">
            ...
            <form action="/Project/MAIN temp/template/pages/dynamic/check.php" method="POST" class="forms-sample" onsubmit="return handleFormSubmit()">
                <div class="form-group">
                    <label for="CHECK_ID" class="form-label"><h5 class="attribute-label">CHECK ID</h5></label>
                    <input type="text" name="CHECK_ID" class="form-control" id="CHECK_ID" placeholder="Enter Check ID">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="AIRCRAFT_TYPE" class="form-label"><h5 class="attribute-label">Aircraft Type</h5></label>
                    <input type="text" name="AIRCRAFT_TYPE" class="form-control" id="AIRCRAFT_TYPE" placeholder="Enter Aircraft Type">
                </div>
                <div class="form-group">
                    <label for="PLANNED_START" class="form-label"><h5 class="attribute-label">Planned Start</h5></label>
                    <input type="datetime-local" name="PLANNED_START" class="form-control" id="PLANNED_START">
                </div>
                <div class="form-group">
                    <label for="PLANNED_END" class="form-label"><h5 class="attribute-label">Planned End</h5></label>
                    <input type="datetime-local" name="PLANNED_END" class="form-control" id="PLANNED_END">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="MAINTENANCE" class="form-label"><h5 class="attribute-label">Maintenance</h5></label>
                    <input type="text" name="MAINTENANCE" class="form-control" id="MAINTENANCE" placeholder="Enter Maintenance">
                </div>
                <div class="form-group">
                    <label for="MAINTENANCE_PROGRAM" class="form-label"><h5 class="attribute-label">Maintenance Program</h5></label>
                    <input type="text" name="MAINTENANCE_PROGRAM" class="form-control" id="MAINTENANCE_PROGRAM" placeholder="Enter Maintenance program">
                </div>
...
<script>
    function handleFormSubmit() {
        ...
        var plannedStart = document.getElementById("PLANNED_START").value;
        var plannedEnd = document.getElementById("PLANNED_END").value;
        var description = document.getElementById("DESCRIPTION").value;
        var maintenance = document.getElementById("MAINTENANCE").value;
        var maintenanceProgram = document.getElementById("MAINTENANCE_PROGRAM").value;

        // Check if any of the fields are empty
        if (checkID.trim() === '' || workOrderID.trim() === '' || aircraftType.trim() === '' || plannedStart.trim() === '' || plannedEnd.trim() === '' || description.trim() === '' || maintenance.trim() === '' || maintenanceProgram.trim() === '') {
            alert("Please fill in all fields.");
            return false; // Prevent form submission
        }
        // Handle the form submission here, if needed
        alert("Your Check is added.");

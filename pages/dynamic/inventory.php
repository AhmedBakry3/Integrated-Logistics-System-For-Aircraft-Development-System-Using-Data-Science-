...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['ADD'])) {
        ...
        try {
            ...
            $stmt = $dbh->prepare("INSERT INTO INVENTORY (PART_NAME,SUPPLY_ID, QUANTITY, INVENTORY_ID, Date_Of_Manufacture, Expire_Date) VALUES (:PART_NAME,:SUPPLY_ID, :QUANTITY, :INVENTORY_ID, :DATE_OF_MANUFACTURE, :EXPIRE_DATE)");

            // Bind parameters and execute the statement
            $stmt->bindParam(':PART_NAME', $PART_NAME);
            $stmt->bindParam(':SUPPLY_ID', $SUPPLY_ID);
...
<li class="nav-item"><a class="nav-link" href="../../pages/dynamic/Data miner.php" style="color: #E74C3C; font-weight: bold; font-size: 13.5px; color: #283593 !important;">Prediction</a></li>
    ...
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Search for Inventory</h5>
            <p class="card-description">Enter the ID you want to search for:</p>
            <form action="\Project\MAIN temp\template\pages\dynamic\inventory.php" method="post">
       ...
       $password = 'YOUR_PASSWORD'; // Oracle password
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_id'])) {
                ...
                try {
                    ...
                    $stmt = $dbh->prepare("SELECT PART_NAME, SUPPLY_ID, QUANTITY, INVENTORY_ID, DATE_OF_MANUFACTURE, EXPIRE_DATE FROM INVENTORY WHERE INVENTORY_ID = :search_id");
                    $stmt->bindParam(':search_id', $search_id);
                    $stmt->execute();

                    // Display the search results
                    echo "<div style='overflow: auto; width: 1300px; height: 110px;'>";
                    echo "<table border='1' class='INVENTORY'>";
                    echo "<tr><th>PART_NAME</th><th>SUPPLY_ID</th><th>QUANTITY</th><th>INVENTORY_ID</th><th>DATE_OF_MANUFACTURE</th><th>EXPIRE_DATE</th></tr>";
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        echo "<tr><td>{$row['PART_NAME']}</td><td>{$row['SUPPLY_ID']}</td><td>{$row['QUANTITY']}</td><td>{$row['INVENTORY_ID']}</td><td>{$row['DATE_OF_MANUFACTURE']}</td><td>{$row['EXPIRE_DATE']}</td></tr>";
                    }
                    echo "</table>";
                    echo "</div>";
...
<div class="card">
        <div class="card-body">
            <h5 class="card-title">Search for Part Name</h5>
            <p class="card-description">Enter the part name you want to search for:</p>
            <form action="\Project\MAIN temp\template\pages\dynamic\inventory.php" method="post">
                <div class="input-group mb-3">
                    <input type="text" class="form-control" name="search_by_part_name" placeholder="Enter ID to search">
                    <button type="submit" class="btn btn-primary">Search</button>
                </div>
            </form>
            ...
            // Check if the search form has been submitted
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_by_part_name'])) {
                $search_id = $_POST['search_by_part_name'];
                ...
                try {
                    ...
                    // Prepare SQL statement to fetch rows matching the specified ID
                    $stmt = $dbh->prepare("SELECT PART_NAME, SUPPLY_ID, QUANTITY, INVENTORY_ID, DATE_OF_MANUFACTURE, EXPIRE_DATE FROM INVENTORY WHERE PART_NAME = :search_by_part_name");
                    $stmt->bindParam(':search_by_part_name', $search_id);
                    $stmt->execute();
...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Delete Part with Part Name</h5>
            <p class="card-description">Enter the Part Name you want to delete:</p>
            <form action="\Project\MAIN temp\template\pages\dynamic\inventory.php" method="post">
                <div class="input-group mb-3">
        ...
        $password = 'YOUR_PASSWORD'; // Oracle password
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_id'])) {
                ...
                try {
                    ...
                    $stmt = $dbh->prepare("DELETE FROM INVENTORY WHERE PART_NAME = :delete_id");
        ...
        <div class="input-group mb-3">
           ...
           $password = 'YOUR_PASSWORD'; // Oracle password
                ...
                try {
                    ...
                    // Query to fetch inventory attributes from the database
                    $stmt = $dbh->query("SELECT PART_NAME, SUPPLY_ID, QUANTITY, INVENTORY_ID, DATE_OF_MANUFACTURE, EXPIRE_DATE FROM INVENTORY"); // Replace 'INVENTORY' with your actual table name
                    ...
                    // Display inventory attributes
                    echo "<div style='overflow: auto; width: 1300px; height: 350px;'>";
                    echo "<table border='1' class='INVENTORY'>";
                    echo "<tr><th>PART_NAME</th><th>SUPPLY_ID</th><th>QUANTITY</th><th>INVENTORY_ID</th><th>DATE_OF_MANUFACTURE</th><th>EXPIRE_DATE</th></tr>";
                    foreach ($rows as $row) {
                        echo "<tr><td>{$row['PART_NAME']}</td><td>{$row['SUPPLY_ID']}</td><td>{$row['QUANTITY']}</td><td>{$row['INVENTORY_ID']}</td><td>{$row['DATE_OF_MANUFACTURE']}</td><td>{$row['EXPIRE_DATE']}</td></tr>";
...
<div class="container mt-5">
    <div class="card">
        <div class="card-body">
            ...
            <form action="/Project/MAIN temp/template/pages/dynamic/inventory.php" method="POST" class="forms-sample" onsubmit="return handleFormSubmit()">
                ...
                <div class="form-group">
                    <label for="INVENTORY_ID" class="form-label"><h5 class="attribute-label">INVENTORY ID</h5></label>
                    <input type="text" name="INVENTORY_ID" class="form-control" id="INVENTORY_ID" placeholder="Enter Inventory ID">
                </div>
                <div class="form-group">
...
<!-- <script>
    function handleFormSubmit() {
      ...
      var supplyid = document.getElementById("Supply_ID").value;
        var Quantity = document.getElementById("QUANTITY").value;
        var inventoryID = document.getElementById("INVENTORY_ID").value;
        ...
        if (partName.trim() === '' || supplyid.trim() === '' || Quantity.trim() === '' || inventoryID.trim() === '' || dateOfManufacture.trim() === '' || expireDate.trim() === '') {
            alert("Please fill in all fields.");
            return false; // Prevent form submission
        }
        
        // Handle the form submission here, if needed
        alert("Your Inventory is added.");

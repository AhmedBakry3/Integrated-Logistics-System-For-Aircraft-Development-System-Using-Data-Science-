...
<head>
  ...
  <title>Component Life Tracking</title>
  <!-- plugins:css -->
...
<style>
    ...
    .COMPONENT_LIFE_TRACKING tr:last-child td {
        border-bottom: none; /* Remove bottom border from last row */
    }

    .COMPONENT_LIFE_TRACKING tr:nth-child(even) {
...
COMPONENT LIFE TRACKING</span></h1>
          </li>
        </ul>
...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Search By Aircraft ID</h5>
            <p class="card-description">Enter the ID you want to search for:</p>
            <form action="\Project\MAIN temp\template\pages\dynamic\component_life_tracking.php" method="post">
...
$password = 'YOUR_PASSWORD'; // Oracle password
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_id'])) {
                    ...
                    try {
                        ...
                        $stmt = $dbh->prepare("SELECT COMPONENT_LIFE_TRACKING_ID, AIRCRAFT_ID, ATA_CHAPTER, SERIAL_NUMBER, COMPONENT_NAME, DATE_OF_MANUFACTURE, OIC, OIV, CHECK_ FROM COMPONENT_LIFE_TRACKING WHERE AIRCRAFT_ID = :search_id");
                        $stmt->bindParam(':search_id', $search_id);
                        $stmt->execute();
                        // Display the search results
                        echo "<div style='overflow: auto; width: 1250px; height: 350px;'>";
                        echo "<table border='1' class='COMPONENT_LIFE_TRACKING'>";
                        echo "<tr><th>COMPONENT_LIFE_TRACKING_ID</th><th>AIRCRAFT_ID</th><th>ATA_CHAPTER</th><th>SERIAL_NUMBER</th><th>COMPONENT_NAME</th><th>DATE_OF_MANUFACTURE</th><th>OIC</th><th>OIV</th><th>CHECK_</th></tr>";
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            echo "<tr><td>{$row['COMPONENT_LIFE_TRACKING_ID']}</td><td>{$row['AIRCRAFT_ID']}</td><td>{$row['ATA_CHAPTER']}</td><td>{$row['SERIAL_NUMBER']}</td><td>{$row['COMPONENT_NAME']}</td><td>{$row['DATE_OF_MANUFACTURE']}</td><td>{$row['OIC']}</td><td>{$row['OIV']}</td><td>{$row['CHECK_']}</td></tr>";
                        }
                        echo "</table>";
                        echo "</div>";
...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Search By Component Life Tracking ID</h5>
            <p class="card-description">Enter the ID you want to search for:</p>
            <form action="\Project\MAIN temp\template\pages\dynamic\component_life_tracking.php" method="post">
                <div class="input-group mb-3">
                    <input type="text" class="form-control" name="search_by_component_id" placeholder="Enter ID to search">
                    <button type="submit" class="btn btn-primary">Search</button>
                </div>
            </form>
            <?php
             ...
             $password = 'YOUR_PASSWORD'; // Oracle password
                ...
                // Check if the search form has been submitted
                if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_by_component_id'])) {
                    $search_by_component_id = $_POST['search_by_component_id'];
                    ...
                    try {
                        ...
                        $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                        // Prepare SQL statement to fetch rows matching the specified ID
                        $stmt = $dbh->prepare("SELECT COMPONENT_LIFE_TRACKING_ID, AIRCRAFT_ID, ATA_CHAPTER,ATA_NAME, SERIAL_NUMBER, COMPONENT_NAME, DATE_OF_MANUFACTURE, OIC, OIV, CHECK_ FROM COMPONENT_LIFE_TRACKING WHERE COMPONENT_LIFE_TRACKING_ID = :search_by_component_id");
                        $stmt->bindParam(':search_by_component_id', $search_by_component_id);
                        $stmt->execute();
                        // Display the search results
                        echo "<div style='overflow: auto; width: 1250px; height: 170px;'>";
                        echo "<table border='1' class='COMPONENT_LIFE_TRACKING'>";
                        echo "<tr><th>COMPONENT_LIFE_TRACKING_ID</th><th>AIRCRAFT_ID</th><th>ATA_CHAPTER</th><th>ATA_NAME</th><th>SERIAL_NUMBER</th><th>COMPONENT_NAME</th><th>DATE_OF_MANUFACTURE</th><th>OIC</th><th>OIV</th><th>CHECK_</th></tr>";
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            echo "<tr><td>{$row['COMPONENT_LIFE_TRACKING_ID']}</td><td>{$row['AIRCRAFT_ID']}</td><td>{$row['ATA_CHAPTER']}</td><td>{$row['ATA_NAME']}</td><td>{$row['SERIAL_NUMBER']}</td><td>{$row['COMPONENT_NAME']}</td><td>{$row['DATE_OF_MANUFACTURE']}</td><td>{$row['OIC']}</td><td>{$row['OIV']}</td><td>{$row['CHECK_']}</td></tr>";
                        }
                        echo "</table>";
                        echo "</div>";
                        // Close the connection
                ...
                <?php
                 ...
                 $password = 'YOUR_PASSWORD'; // Oracle password
                    ...
                    try {
                        ...
                        // Set PDO error mode to exception
                        $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                        // Query to fetch component attributes from the database
                        $stmt = $dbh->query("SELECT COMPONENT_LIFE_TRACKING_ID, AIRCRAFT_ID, ATA_CHAPTER,ATA_NAME, SERIAL_NUMBER, COMPONENT_NAME, DATE_OF_MANUFACTURE, OIC, OIV, CHECK_ FROM COMPONENT_LIFE_TRACKING");
...
<div class="card">
    <div class="card-body">
        <div class="card-body">
            <h5 class="card-title">Component Life Tracking</h5>
            <p class="card-description">
                Component Life Tracking Info
            </p>
            <form action="\Project\MAIN temp\template\pages\dynamic\component_life_tracking.php" method="POST" class="forms-sample" onsubmit="return handleFormSubmit()">
                <div class="form-group">
                    <label for="exampleInputName1">Component Life Tracking ID</label>
                    <input type="text" name="COMPONENT_LIFE_TRACKING_ID" class="form-control" id="COMPONENT_LIFE_TRACKING_ID" placeholder="COMPONENT_LIFE_TRACKING_ID">
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail3">Component ID</label>
                    <input type="text" name="COMPONENT_ID" class="form-control" id="COMPONENT_ID" placeholder="COMPONENT_ID">
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail3">OIC</label>
                    <input type="text" name="OIC" class="form-control" id="OIC" placeholder="OIC">
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail3">OIV</label>
                    <input type="text" name="OIV" class="form-control" id="OIV" placeholder="OIV">
                </div>
...
<script>
    function handleFormSubmit() {
        var componentLifeTrackingID = document.getElementById("COMPONENT_LIFE_TRACKING_ID").value;
        var componentID = document.getElementById("COMPONENT_ID").value;
        var OIC = document.getElementById("OIC").value;
        var OIV = document.getElementById("OIV").value;

        // Check if any of the fields are empty
        if (componentLifeTrackingID.trim() === '' || componentID.trim() === '' || OIC.trim() === '' || OIV.trim() === '') {
            alert("Please fill in all fields.");
            return false; // Prevent form submission
        }

        // Handle the form submission here, if needed
        alert("Your Component Life Tracking data is added.");

...
<style>
    ...
    .FAULT_TABLE tr:last-child td {
        border-bottom: none; /* Remove bottom border from last row */
    }

    .FAULT_TABLE tr:nth-child(even) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['ADD'])) {
        ...
        try {
            ...
            $DATE_OF_FAULT = $_POST['DATE_OF_FAULT'];
            $HANGAR = $_POST['HANGAR'];

            // Prepare SQL statement to insert data into database
            $stmt = $dbh->prepare("INSERT INTO FAULT (FAULT_ID, WORK_ORDER_ID, FAULT_CODE, AC_TYPE, AC_REGISTER, FAULT_DESCRIPTION, DATE_OF_FAULT, HANGAR) 
                VALUES (:FAULT_ID, :WORK_ORDER_ID, :FAULT_CODE, :AC_TYPE, :AC_REGISTER, :FAULT_DESCRIPTION,:DATE_OF_FAULT , :HANGAR)");

            // Bind parameters and execute the statement
            $stmt->bindParam(':FAULT_ID', $FAULT_ID);
            $stmt->bindParam(':WORK_ORDER_ID', $WORK_ORDER_ID);
            $stmt->bindParam(':FAULT_CODE', $FAULT_CODE);
            $stmt->bindParam(':AC_TYPE', $AC_TYPE);
            $stmt->bindParam(':AC_REGISTER', $AC_REGISTER);
            $stmt->bindParam(':FAULT_DESCRIPTION', $FAULT_DESCRIPTION);
            $stmt->bindParam(':DATE_OF_FAULT', $DATE_OF_FAULT);
            ...
            echo "Fault record added successfully!";
...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Search for Fault</h5>
            <p class="card-description">Enter the ID you want to search for:</p>
            <form action="\Project\MAIN temp\template\pages\dynamic\Fault.php" method="post">
                <div class="input-group mb-3">
                    <input type="text" class="form-control" name="search_id" placeholder="Enter Fault ID to search">
...
$password = 'YOUR_PASSWORD'; // Oracle password
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_id'])) {
                ...
                try {
                    ...
                    $stmt = $dbh->prepare("SELECT FAULT_ID, WORK_ORDER_ID, FAULT_CODE, AC_TYPE, AC_REGISTER, FAULT_DESCRIPTION, DATE_OF_FAULT, HANGAR FROM FAULT WHERE FAULT_ID = :search_id");
                    $stmt->bindParam(':search_id', $search_id);
                    $stmt->execute();

                    // Display the search results
                    echo "<div style='overflow: auto; width: 1250px; height: 170px;'>";
                    echo "<table border='1' class='FAULT_TABLE'>";
                    echo "<tr><th>FAULT_ID</th><th>WORK_ORDER_ID</th><th>FAULT_CODE</th><th>AC_TYPE</th><th>AC_REGISTER</th><th>FAULT_DESCRIPTION</th><th>DATE_OF_FAULT</th><th>HANGAR</th></tr>";
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        echo "<tr><td>{$row['FAULT_ID']}</td><td>{$row['WORK_ORDER_ID']}</td><td>{$row['FAULT_CODE']}</td><td>{$row['AC_TYPE']}</td><td>{$row['AC_REGISTER']}</td><td>{$row['FAULT_DESCRIPTION']}</td><td>{$row['DATE_OF_FAULT']}</td><td>{$row['HANGAR']}</td></tr>";
                    }
                    echo "</table>";
                    echo "</div>";
...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Delete fault with ID</h5>
       ...
       $password = 'YOUR_PASSWORD'; // Oracle password
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_id'])) {
                ...
                try {
                    ...
                    $stmt = $dbh->prepare("DELETE FROM FAULT WHERE FAULT_ID = :delete_id");
...
$password = 'YOUR_PASSWORD'; // Oracle password
                ...
                try {
                    ...
                    // Query to fetch fault attributes from the database
                    $stmt = $dbh->query("SELECT FAULT_ID, WORK_ORDER_ID, FAULT_CODE, AC_TYPE, AC_REGISTER, FAULT_DESCRIPTION, DATE_OF_FAULT, HANGAR FROM FAULT");
                    
                    // Store rows in an array
                    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    
                    // Display fault attributes in a table
...
<div class="container mt-5">
    <div class="card">
        <div class="card-body">
            ...
            <form action="/Project/MAIN temp/template/pages/dynamic/Fault.php" method="POST" class="forms-sample" onsubmit="return handleFormSubmit()">
                <div class="form-group">
                    <label for="FAULT_ID" class="form-label"><h5 class="attribute-label">FAULT ID</h5></label>
                    <input type="text" name="FAULT_ID" class="form-control" id="FAULT_ID" placeholder="Enter Fault ID">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="FAULT_CODE" class="form-label"><h5 class="attribute-label">Fault Code</h5></label>
                    <input type="text" name="FAULT_CODE" class="form-control" id="FAULT_CODE" placeholder="Enter Fault Code">
                </div>
                <div class="form-group">
                    <label for="AC_TYPE" class="form-label"><h5 class="attribute-label">Aircraft Type</h5></label>
                    <input type="text" name="AC_TYPE" class="form-control" id="AC_TYPE" placeholder="Enter Aircraft Type">
                </div>
                <div class="form-group">
                    <label for="AC_REGISTER" class="form-label"><h5 class="attribute-label">Aircraft Register</h5></label>
                    <input type="text" name="AC_REGISTER" class="form-control" id="AC_REGISTER" placeholder="Enter Aircraft Register">
                </div>
                <div class="form-group">
                    <label for="FAULT_DESCRIPTION" class="form-label"><h5 class="attribute-label">Fault Description</h5></label>
                    <textarea name="FAULT_DESCRIPTION" class="form-control" id="FAULT_DESCRIPTION" rows="3" placeholder="Enter Fault Description"></textarea>
                </div>
                <div class="form-group">
                    <label for="DATE_OF_FAULT" class="form-label"><h5 class="attribute-label">Date of Fault</h5></label>
                    <input type="datetime-local" name="DATE_OF_FAULT" class="form-control" id="DATE_OF_FAULT">
                </div>
                <div class="form-group">
                    <label for="HANGAR" class="form-label"><h5 class="attribute-label">Hangar</h5></label>
                    <input type="text" name="HANGAR" class="form-control" id="HANGAR" placeholder="Enter Hangar">
                </div>
...
<script>
    function handleFormSubmit() {
        var faultID = document.getElementById("FAULT_ID").value;
        var workOrderID = document.getElementById("WORK_ORDER_ID").value;
        var faultCode = document.getElementById("FAULT_CODE").value;
        var aircraftType = document.getElementById("AC_TYPE").value;
        var aircraftRegister = document.getElementById("AC_REGISTER").value;
        var faultDescription = document.getElementById("FAULT_DESCRIPTION").value;
        var dateOfFault = document.getElementById("DATE_OF_FAULT").value;
        var hangar = document.getElementById("HANGAR").value;

        // Check if any of the fields are empty
        if (faultID.trim() === '' || workOrderID.trim() === '' || faultCode.trim() === '' || aircraftType.trim() === '' || aircraftRegister.trim() === '' || faultDescription.trim() === '' || dateOfFault.trim() === '' || hangar.trim() === '') {
            alert("Please fill in all fields.");
            return false; // Prevent form submission
        }
        // Handle the form submission here, if needed
        alert("Your Fault is added.");
